...
ggplot(df.melt,aes(x=x,y=value,fill=variable))+geom_bar(stat="identity")+scale_y_continuous("Cara - Barata",formatter="comma",limits=c(0,200))+opts(axis.text.x=theme_text(size=20))+opts(axis.text.y=theme_text(size=20))
dev.off()